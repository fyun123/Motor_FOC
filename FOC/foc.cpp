...
void svm(float v_bus, float u, float v, float w, float *dtc_u, float *dtc_v, float *dtc_w){
    ...
    *dtc_u = fminf(fmaxf(((u -v_offset)/v_bus + .5f), DTC_MIN), DTC_MAX);   //u=(u -v_offset)/v_bus + 0.5,limited  between min and max
...
void init_controller_params(ControllerStruct *controller){
    controller->ki_d = KI_D;  //0.0255
    controller->ki_q = KI_Q;  //0.0255
    controller->k_d = K_SCALE*I_BW;          //k_d=0.0001*Current loop bandwidth
    controller->k_q = K_SCALE*I_BW;          //k_q=0.0001*Current loop bandwidth
    controller->alpha = 1.0f - 1.0f/(1.0f - DT*I_BW*2.0f*PI);   //                            1
                                                                //alpha=1 - ---------------------------------------
	                                                              //           1-0.000025*Current loop bandwidth*2pi
    }
...
void reset_foc(ControllerStruct *controller){
    ...
    controller->q_int = 0;    //q Current error integrals
    controller->d_int = 0;    //d Current error integrals 
    controller->v_q = 0;
    controller->v_d = 0;
	controller->v_int = 0;
	controller->i_ref = 0;
	controller->satError = 0;
...
void limit_current_ref (ControllerStruct *controller){
    ...
    controller->i_q_ref = fmaxf(fminf(i_q_max_limit, controller->i_q_ref), i_q_min_limit);  //iq ref limit
    }
...
void commutate(ControllerStruct *controller, ObserverStruct *observer, GPIOStruct *gpio, float theta){
        ...
        observer->resistance = (controller->v_q - SQRT3*controller->dtheta_elec*(WB))/controller->i_q;  //Ro=(vq-aqrt3*¦Èe*flux)/iq
        //observer->resistance = controller->v_q/controller->i_q;
        if(isnan(observer->resistance)){observer->resistance = R_PHASE;}    //if resistance is nan,resistance=phase resistance
        observer->temperature2 = (double)(25.0f + ((observer->resistance*6.0606f)-1.0f)*275.5f);  //T=25+(6.0606*Ro-1)*275.5
        double e = observer->temperature - observer->temperature2;    //temperature error
        observer->temperature -= .001*e;      //Correct estimated temperature
        //printf("%.3f\n\r", e);
       ...
       //controller->i_q = 0.6666667f*(-s*controller->i_a - (-0.86602540378f*c-.5f*s)*controller->i_b - (0.86602540378f*c-.5f*s)*controller->i_c);
        //my_dac.write((controller->i_q)/12);
...
//       limit_norm(&controller->i_d_ref, &controller->i_q_ref, I_MAX);
        ...
        // ==== Èõ´Å¿ØÖÆ 1===== //
//			 float fw_error = Mth - controller->v_ref / (controller->v_bus / SQRT3);
//			 controller->fw_int += Ki_fw * fw_error;
//			 controller->i_d_ref = Kp_fw * fw_error + controller->fw_int;
//			 controller->i_d_ref = fmaxf(fminf(controller->i_d_ref, 0.0f), -I_FW_MAX);
//			 limit_norm(&controller->i_d_ref, &controller->i_q_ref, I_MAX);
			 
//			 // ===== Èõ´Å¿ØÖÆ 2 === //
//			 float fw_error = Mth - controller->v_ref / (controller->v_bus / SQRT3);
//			 controller->fw_int += Ki_fw * fw_error;
//			 float satOutPre = controller->fw_int + Kc_fw * controller->satError;
//			 float satOut = fmaxf(fminf(satOutPre, 0.0f), 1.0f);
//			 controller->satError = (satOut - satOutPre);
//			 //  ³¬Ç°½Ç
//			 float idq_theta = atan(controller->i_q_ref / controller->i_d_ref);
//			 idq_theta = PI - ((PI - idq_theta) * satOut);
//			 controller->i_d_ref = controller->i_ref * cos(idq_theta);
//			 controller->i_q_ref = controller->i_ref * sin(idq_theta);
//			 limit_norm(&controller->i_d_ref, &controller->i_q_ref, I_MAX);
//       controller->i_ref = sqrt(controller->i_d_ref * controller->i_d_ref + controller->i_q_ref * controller->i_q_ref);
//			 
//       
       /// PI Controller ///
       float i_d_error = controller->i_d_ref - controller->i_d_filt;
       float i_q_error = controller->i_q_ref - controller->i_q_filt;//  + cogging_current;
       ...
       svm(controller->v_bus, controller->v_u, controller->v_v, controller->v_w, &controller->dtc_u, &controller->dtc_v, &controller->dtc_w); //space vector modulation
			 if(theta >= 0 && theta < PI/3){
				 controller->dtc_v += 2.5 * 0.008f;
			 }else if (theta >= PI/3 && theta < 2*PI/3){
				 controller->dtc_u -= 2.5 * 0.008f;
			 }else if(theta >= 2*PI/3 && theta < PI){
				 controller->dtc_w += 2.5 * 0.008f;
...
//    float torque_ref = controller->kp*(controller->p_des - controller->theta_mech) + controller->t_ff + controller->kd*(controller->v_des - controller->dtheta_mech);
		float error = controller->v_des - controller->dtheta_mech;
		controller->v_int += controller->kd * error;
		controller->v_int = fmaxf(fminf(controller->v_int, 8.0f), -8.0f);
		controller->i_q_ref = controller->kp * error + controller->v_int + controller->t_ff / 0.32f;

...
//can
CAN          can(CAN_RX_PINNAME, CAN_TX_PINNAME, CAN_FREQUENCY); 
CANMessage   rxMsg;
CANMessage   txMsg;
//DRV8323
SPI drv_spi(DRV_MOSI_PINNAME, DRV_MISO_PINNAME, DRV_SCLK_PINNAME);
...
int main()
{
    ...
    if(isnan(E_OFFSET))
    {
        E_OFFSET = 0.0f;   //Returns TRUE if x is a NaN.Encoder electrical offset
    }

    if(isnan(M_OFFSET))
    {
        M_OFFSET = 0.0f;   //Encoder mechanical offset
    }

    if(isnan(I_BW) || I_BW == -1 || I_BW == 0)
    {
        I_BW = 1000;   // Current loop bandwidth
    }

    if(isnan(I_MAX) || I_MAX == -1 || I_MAX == 0)
    {
        I_MAX = 40;   // Torque limit (current limit = torque_limit/(kt*gear ratio))
    }

    if(isnan(I_FW_MAX) || I_FW_MAX == -1)
    {
        I_FW_MAX = 0;   // Maximum field weakening current
    }

    if(isnan((float)CAN_ID) || CAN_ID == -1)
    {
        CAN_ID = 1;   // CAN bus ID
    }

    if(isnan((float)CAN_MASTER) || CAN_MASTER == -1)
    {
        CAN_MASTER = 0;   // CAN bus "master" ID
    }

    if(isnan((float)CAN_TIMEOUT) || CAN_TIMEOUT == -1)
    {
    ...
    posSensor.WriteLUT(lut);                                                          // Set potision sensor nonlinearity lookup table
		speedController.initController();
